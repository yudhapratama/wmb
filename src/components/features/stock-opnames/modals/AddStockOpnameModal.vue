  ...
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
      ...
      <div class="flex justify-between items-center p-6 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Tambah Stock Opname Baru</h2>
        <button
          @click="$emit('close')"
      ...
      <div class="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
            <div class="flex">
              <InformationCircleIcon class="w-5 h-5 text-blue-400 mt-0.5 mr-3" />
              <div class="text-sm text-blue-700">
                <p class="font-medium mb-1">Informasi:</p>
                <ul class="list-disc list-inside space-y-1">
                  <li>Setelah membuat stock opname, Anda dapat menambahkan item-item yang akan dihitung</li>
                  <li>Status "Draft" memungkinkan Anda untuk terus mengedit dan menambah item</li>
                  <li>Status "Selesai" akan mengunci data dan memproses koreksi stok otomatis</li>
                </ul>
              </div>
            </div>
          </div>
      ...
      <div class="flex justify-end space-x-3 p-6 border-t border-gray-200 bg-gray-50">
        ...
        >
          ...
          {{ isLoading ? 'Menyimpan...' : 'Simpan Stock Opname' }}
        </button>
      </div>
    </div>
  </div>
...
// Props will be defined below with const props for watcher
...
// Form data
const form = ref({
  tanggal_opname: new Date().toISOString().split('T')[0], // Today's date
  status: 'draft',
  catatan_keseluruhan: ''
})
...
// Get props reference for watcher
